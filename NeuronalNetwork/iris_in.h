...
}   size    =  {.vertical=6, .ordonnencement=500, .affinage=1500, .alpha_ordonn= 0.9, .alpha_affin = 0.09}; 

char*       categories[] = {"Iris-setosa", "Iris-versicolor", "Iris-virginica", "unknown"};
...
//Add new flower to identify
void addNewFlower(DataIris *data){
    srand(time(NULL));
    int random = rand()%(150-0)+0;
    float a,b,c,d;
    printf("Entrer the values of the new flower to classify:");
    printf("\nSepalLength SepalWidth PepalLength PepalWidth: ");
    scanf("%f %f %f %f", &a,&b,&c,&d);
    data[random].dataIris[0] = a;
    data[random].dataIris[1] = b;
    data[random].dataIris[2] = c;
    data[random].dataIris[3] = d;
    data[random].nameIris = "unknown"; 
}
